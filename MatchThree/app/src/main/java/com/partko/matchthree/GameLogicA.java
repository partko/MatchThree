...
public class GameLogicA {
    ...
    int[][] boxMatrix = new int[8][8];

    protected boolean isBoxCrack = false;

    GameLogicA() {
        //---------------- маски поиска ----------------//
        searchMasks[0][0][0] = searchMasks[0][0][1] = searchMasks[0][1][2] = 1; searchMasks[0][0][2] = 2;
        searchMasks[1][1][0] = searchMasks[1][1][1] = searchMasks[1][0][2] = 1; searchMasks[1][1][2] = 2;
        searchMasks[2][0][0] = searchMasks[2][1][1] = searchMasks[2][0][2] = 1; searchMasks[2][0][1] = 2;
        searchMasks[3][1][0] = searchMasks[3][0][1] = searchMasks[3][0][2] = 1; searchMasks[3][0][0] = 2;
        searchMasks[4][0][0] = searchMasks[4][1][1] = searchMasks[4][1][2] = 1; searchMasks[4][1][0] = 2;
        searchMasks[5][1][0] = searchMasks[5][0][1] = searchMasks[5][1][2] = 1; searchMasks[5][1][1] = 2;
        searchMasks[6][2][0] = searchMasks[6][0][1] = searchMasks[6][1][1] = 1; searchMasks[6][2][1] = 2;
        searchMasks[7][0][0] = searchMasks[7][1][0] = searchMasks[7][2][1] = 1; searchMasks[7][2][0] = 2;
        searchMasks[8][0][0] = searchMasks[8][1][1] = searchMasks[8][2][1] = 1; searchMasks[8][0][1] = 2;
        searchMasks[9][1][0] = searchMasks[9][2][0] = searchMasks[9][0][1] = 1; searchMasks[9][0][0] = 2;
        searchMasks[10][1][0] = searchMasks[10][0][1] = searchMasks[10][2][1] = 1; searchMasks[10][1][1] = 2;
        searchMasks[11][0][0] = searchMasks[11][2][0] = searchMasks[11][1][1] = 1; searchMasks[11][1][0] = 2;
        searchMasks[12][0][0] = searchMasks[12][0][1] = searchMasks[12][0][3] = 1; searchMasks[12][0][2] = 2;
        searchMasks[13][0][0] = searchMasks[13][0][2] = searchMasks[13][0][3] = 1; searchMasks[13][0][1] = 2;
        searchMasks[14][0][0] = searchMasks[14][1][0] = searchMasks[14][3][0] = 1; searchMasks[14][2][0] = 2;
        searchMasks[15][0][0] = searchMasks[15][2][0] = searchMasks[15][3][0] = 1; searchMasks[15][1][0] = 2;
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if (boxMatrix[i][j] == 1) {
                    ...
                    matrix[i][j] = random.nextInt(8) + 1; //gems.length = 8
                }
            }
        }
    }
    ...
    protected boolean searchMatches(boolean addToScore) {
        ...
        for (int y = 0; y < 8; y++) { // поле 8х8
            ...
            for (int x = 0; x < 8; x++) {
                ...
                if (abs(matrix[x][y]) == ChBlock && abs(matrix[x][y]) <= 8) { // ChBlock - число группы
                    NChBlock += 1;
                } else {
    ...
    protected boolean checkMatrix(int x1, int y1, int x2, int y2) {
        Log.d(TAG, "Вызван checkMatrix()");
    ...
    //---------------- проверка на слитие коробок ----------------//
    protected void boxes() {
        Log.d(TAG, "Вызван boxes()");
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if (matrix[i][j] == 9){
                    if (matrix[Math.min(i+1, 7)][j] < 0 && matrix[Math.min(i+1, 7)][j] != -9 || matrix[Math.max(i-1, 0)][j] < 0 && matrix[Math.max(i-1, 0)][j] != -9 || matrix[i][Math.min(j+1, 7)] < 0 && matrix[i][Math.min(j+1, 7)] != -9 || matrix[i][Math.max(j-1, 0)] < 0 && matrix[i][Math.max(j-1, 0)] != -9) {
                        matrix[i][j] = -9;
                        boxMatrix[i][j] = 0;
                        isBoxCrack = true;
                        score += 10; // 10 очков за разбитие коробки
                    }
                }
            }
        }
        displayMatrix(boxMatrix);
    }
                    ...
                    for (int x = 0; x < (8 - maskSizes[mask_number][0] + 1); x++) {
                        ...
                        for (int My = 0; My < maskSizes[mask_number][1]; My++) {
                            for (int Mx = 0; Mx < maskSizes[mask_number][0]; Mx++) {
                                ...
                                if (searchMasks[mask_number][Mx][My] == 2 && matrix[x + Mx][y + My] == 9) {
                                    Nmask -= 1;
                                }
                            }
                        }
    ...
    protected void fallingGems() {
        ...
        for (int x = 0; x < 8; x++) {
            ...
            boxCount = 0; // Число коробок группой в текщем столбце
            for (int y = 7; y > -1; y--) {
                ...
                if (matrix[x][y] == 9 && NHole >= 1) { // Коробка
                    boxCount += 1; // Подсчет коробок в группе
                }
                if (matrix[x][y] > 0 && NHole > 0 && matrix[x][y] != 9) { // Фишка над пропуском
                    matrix[x][YHole] = matrix[x][y]; // Упавшая фишка
                    if (matrix[x][YHole-1] != 9) {
                        YHole--; // Перемещение самого нижнего пропуска
                    } else {
                        YHole = YHole - 1 - boxCount;
                        boxCount = 0; // Сброс группы коробок
                    }
